...
| `WaterMonitorModule` | [src/water/](src/water/) | `WaterMonitorService`: polls tank/cisterna level, persists and alerts on low level |
| `MonitorModule` | [src/monitor/](src/monitor/) | `GET /monitor/snapshot` — aggregates the in-memory live state of all three monitors (temperature/transfer/water) in one response, for the front's first render (SSR) without waiting for the first WebSocket tick |
